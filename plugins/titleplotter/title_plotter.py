...
/***************************************************************************
 TitlePlotter-PhilippineLandTitles
                                 A QGIS plugin
 Title Plotter - Philippine Land Titles is a custom QGIS plugin designed to
 automate the plotting of land parcels using the official Philippine land
 title format. Built specifically for geodetic engineers, GIS analysts, and
 real estate professionals, this tool allows users to input technical
 descriptions (bearing-distance or metes-and-bounds format) and instantly
 generate accurate parcel geometries.

The plugin intelligently interprets common Philippine survey notations—such
as those from TCTs and OCTs—and supports both PRS92 and WGS84 coordinate
systems. It simplifies boundary plotting by automating bearing parsing,
distance conversion, and closing error checks.

Whether you're working with Cadastral maps, mother lots, or subdivisions,
PH Land Title Plotter provides a reliable and streamlined solution—right
inside QGIS.
...
# Initialize Qt resources from file resources.py. This is wrapped because
# resources.py is compiled with pyrcc5 (``from PyQt5 import QtCore``), which
# raises ImportError on a pure Qt6/PyQt6 QGIS 4 build and would otherwise abort
# the whole plugin load. The toolbar icon is loaded directly (see initGui), so
# the Qt resource path is not required for the plugin to work.
try:
...
class TitlePlotterPhilippineLandTitles:
    ...
    def initGui(self):
        ...
        # Create the action that will start plugin configuration. Prefer the
        # crisp, multi-resolution recolored logo; fall back to the bundled PNG
